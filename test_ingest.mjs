...
// recorded result was removed with the old extended docs.
//
...
const { parseHeaderLine, buildColumnPlan, areaHashes, dayOfYear } = await import(
  './src/ingest/header.ts'
);
const { instantiateParser, parseBytes, afterNextNewline, scanAxis: scanAxisRaw, PARSER_ABI } =
  await import('./src/ingest/block.ts');

/**
 * One byte range through both passes, the way the pool runs them: the scan
 * counts the rows, the parse is bounded by that count.
 */
function parseRange(instance, bytes, from, to, plan, areaCount) {
...
// ---------------------------------------------------------------- variable shapes
//
// The parser reads its shape at runtime, so these are not a second code path
// any more -- they are the same code path with different numbers, and the
// cell-by-cell gate above applies to them unchanged.
...
function syntheticCsv(axis, metricCount, { hours = 2, order = null, pad = '' } = {}) {
  ...
  for (let hour = 1; hour <= hours; hour++) {
    ...
    for (const area of rowOrder) {
      ...
      for (let m = 0; m < metricCount; m++) fields.push(String(code * 100000 + hour * 1000 + m + 1));
...
async function runShape(bytes, axis, retained = null, blockBytes = 4096) {
  ...
  const shapeParser = await instantiateParser(wasmModule, areaHashes(axis));
...
// Fewer areas than the old fixed 43, source metric positions past the old
// fixed 50, and area rows in a different order per hour.
{
  const axis = ['A', 'B', 'C', 'D'];
...
// More areas than the old fixed 43, which also pushes the area hash table past
// the count it used to be sized for.
{
...
// A padded Name field. The header's own key columns ship padded (` Hour`,
// ` TOU`, ` Name`), so a padded value is well within what this format does,
// and the axis is built from TRIMMED names -- the parser has to hash the same
// bytes or a merely-spaced file is refused as "unknown area".
{
  const axis = ['A', 'B', 'C'];
...
// The mapping from source column to slab plane is NOT a function of the
// shape. Two cases can carry identical area and column counts in a different
// column order, and one worker parses blocks from both. If that mapping were
// cached by shape, the second case would be read through the first case's
// column plan -- every retained column silently sourced from the wrong column,
// which is footgun 18 with nothing thrown.
{
  const axis = ['A', 'B', 'C'];
  ...
  for (const retained of [first, second, first]) {
    ...
    for (const [from, to] of ranges) {
      blitBlock(
        accumulator,
        parseRange(shapeParser, bytes, from, to, plan, axis.length),
      );
    }
...
// row list and the blit scatters it, so order carries no meaning at all. This
// used to be refused; now it has to produce the SAME CUBE as the ordered file,
// byte for byte, which is the only claim worth making.
{
  const axis = ['A', 'B', 'C'];
...
// ---------------------------------------------------------------- axis scan
//
// The area axis used to be guessed: read Name from the first rows until one
// repeats. That is wrong whenever the first hour does not list every area
// exactly once, and it failed silently -- a short axis, no complaint. These
// check the axis is read from every row instead.
{
  const axis = ['A', 'B', 'C', 'D'];
  const { metrics, bytes } = syntheticCsv(axis, 5, { hours: 4 });
  // Drop area D from hour 1 only, so the old first-hour rule would miss it.
  const lines = new TextDecoder().decode(bytes).split('\r\n').filter((l) => l.length > 0);
  const thinned = lines.filter((line, i) => !(i > 0 && i <= axis.length && line.split(',')[3] === 'D'));
  ...
  // A scrambled file is not a defect, and the scan must treat it as ordinary:
  // the same complete axis and the same row bound as the ordered file.
  // Reverse the SAME thinned rows the ordered scan above read, so the row
  // bounds are comparable and D is still missing from what was the first hour.
  ...
  assert.deepEqual(
    ...
    axis.slice().sort(),
    'a scrambled file still yields the COMPLETE axis -- the old rule returned a short one',
  );
...
// The committed block.wasm must be the one this build expects. block.wasm is
// checked in, so a stale binary against updated TypeScript is a real way to
// ship a bug -- instantiateParser above already gates on ABI_VERSION, and this
// states the number the source tree is on.
assert.equal(PARSER_ABI, 5);
...
// and looks like plausible MWh. Both are checked against the reference cube.
{
  ...
  for (const [from, to] of run.ranges) {
    blitBlock(
      accumulator,
      parseRange(parser, sampleBytes, from, to, plan, AREA_COUNT),
    );
  }
  ...
  for (const [from, to] of run.ranges) {
    blitBlock(
      partial,
      parseRange(parser, sampleBytes, from, to, without, AREA_COUNT),
    );
  }
  ...
  for (const [from, to] of run.ranges) {
    blitBlock(
      netAcc,
      parseRange(parser, sampleBytes, from, to, netPlan, AREA_COUNT),
    );
  }
